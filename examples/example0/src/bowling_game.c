...
static bool is_spare(struct bowling_game const *game, int frame_index) {
    return game->rolls[frame_index] + game->rolls[frame_index + 1] == 10;
}

static bool is_strike(struct bowling_game const *game, int frame_index) {
    return game->rolls[frame_index] == 10;
}

static int strike_score(struct bowling_game const *game, int frame_index) {
    return 10 + game->rolls[frame_index + 1] + game->rolls[frame_index + 2];
}

static int spare_score(struct bowling_game const *game, int frame_index) {
    return 10 + game->rolls[frame_index + 2];
}

static int normal_score(struct bowling_game const *game, int frame_index) {
